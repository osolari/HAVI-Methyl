...
\paragraph{Joint training inside \texttt{fit\_svi\_torch}.} The variance-weighted Dirichlet head also runs as a joint training term inside the torch SVI loop when the caller passes \texttt{tissue\_reference} (the $T\times L$ panel), \texttt{tissue\_target} (the ground-truth $S\times T$ mixture), and \texttt{tissue\_weight}~$>0$. On each mini-batch the head solves a differentiable \texttt{torch.linalg.lstsq} on the locus subset to recover $\hat\pi$, then adds $\lambda_{\mathrm{ToO}}\|\hat\pi-\pi_{\mathrm{true}}\|^2$ to the total loss. The post-hoc head of Sec.~\ref{sec:benchmark:real} is recovered by leaving \texttt{tissue\_weight}=0.
