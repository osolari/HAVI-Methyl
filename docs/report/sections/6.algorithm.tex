...
We use AdamW learning rate $\alpha_\phi=10^{-3}$ for neural parameters and Robbins-Monro step sizes $\rho_t=(t+1)^{-0.6}$ for moment-matched natural-gradient layers. Mini-batches are planned at $|B_s|=8$ samples and $|B_\ell|=4096$ loci per step. Gradient clipping is applied at norm $5.0$, IWAE sample count is $K=1$ during main training and $K=8$ during fine-tuning, and KL annealing $T_{\mathrm{anneal}}$ is one epoch unless validation suggests otherwise.
