...
\paragraph{Variance reduction.} We apply reparameterization for continuous latents, sticking-the-landing \citep{roeder2017stl} for the encoder gradient, and simple control variates for high-variance discrete or count terms. When the IWAE bound is in use ($K>1$), the released \texttt{fit\_svi\_torch} loop activates the Tucker-2019 doubly-reparameterized gradient estimator via \texttt{TorchSVIConfig.iwae\_dreg} (\S\ref{sec:elbo}). The simplified-numpy harness keeps the empirical-Bayes Gaussian-posterior path; both are exposed end-to-end.
