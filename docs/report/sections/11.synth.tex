...
The synthetic-experiment numbers above are single-seed point estimates at reduced scale, not final benchmark statistics. The released code includes a multi-seed sidecar runner (\texttt{scripts/bench\_multiseed\_recovery.py}) that emits per-coverage median, 5th, and 95th percentile of the recovery metrics across 20 seeds in \texttt{tables/tab\_recovery\_multiseed.csv}; the canonical seed-$20260429$ values in this section fall within those 90\% bands at every coverage. Full ablations of the Set Transformer encoder, normalizing-flow posterior, IWAE objective, VIB penalty, mQTL anchors, conformal wrapper, and tissue head are planned for Section~\ref{sec:benchmark}. Appendix~\ref{app:extres} reports extended metrics from the same released run and records sensitivity analyses that should be regenerated before publication if they are to be treated as completed measurements.
