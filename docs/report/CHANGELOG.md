...
**Headline real-data result.** The BB-trials bug fix (using WGBS read coverage `ds.n_total` rather than WGS fragment count `ds.n` as the Beta-Binomial trials) lifts HAVI-Methyl (full torch) on the Liu 2024 paired panel from r = −0.07 to **r = 0.467 vs FinaleMe-style HMM r = 0.078** (6.0×, 500-iter A10G run; 200-iter intermediate landed r = 0.455), AUC 0.75 vs 0.56, ECE 0.31 vs 0.47. Per-stratum: HAVI is the only method with positive Pearson r in every WGBS-depth stratum (FinaleMe is anti-correlated at r = −0.05 in the multi-read interior). On the Loyfer/UXM\_deconv U25 panel the variance-weighted Dirichlet head wins LOO RMSE at every one of the 36 cell types.
